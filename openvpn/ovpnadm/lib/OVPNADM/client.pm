...
sub formManageClient {
	...
	my $lsth = $self->{DBH}->prepare("SELECT DATE_FORMAT(login,'%b %d %Y %h:%i %p'),state FROM clientlogin WHERE cid = $client->[0] ORDER BY login LIMIT 1");
	$lsth->execute();
